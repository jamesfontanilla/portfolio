...
function CompetitionDetail({ competition, onBack, isMobile }: { competition: Competition; onBack: () => void; isMobile: boolean }) {
  ...
  return (
    <div data-competition-detail="true" style={{ height: isMobile ? 'auto' : '100%', minHeight: isMobile ? '100%' : undefined, overflowY: isMobile ? 'visible' : 'auto', overflowX: 'hidden', boxSizing: 'border-box', minWidth: 0, padding: isMobile ? '16px' : '22px', display: 'flex', flexDirection: 'column', gap: isMobile ? '16px' : '18px' }}>
      ...
      <section data-competition-detail-hero="true" style={{ display: 'grid', gridTemplateColumns: isMobile ? 'minmax(0, 1fr)' : 'minmax(0, 1.4fr) minmax(210px, 0.6fr)', gap: isMobile ? '14px' : '20px', padding: isMobile ? '16px' : '22px', borderRadius: '18px', border: '1px solid rgba(231,194,90,0.2)', background: 'linear-gradient(135deg, rgba(231,194,90,0.12), rgba(112,180,255,0.07)), rgba(255,255,255,0.04)', minWidth: 0 }}>
        <div style={{ display: 'grid', gap: '12px', alignContent: 'start', minWidth: 0 }}><div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}><Tag label={competition.status} gold />{competition.tags.slice(0, 4).map((tag) => <Tag key={tag} label={tag} />)}</div><h2 style={{ margin: 0, maxWidth: isMobile ? '100%' : '14ch', color: 'var(--text)', fontFamily: '"Space Grotesk","Manrope",sans-serif', fontSize: isMobile ? 'clamp(1.8rem, 11vw, 2.8rem)' : 'clamp(1.8rem, 4vw, 3.8rem)', fontWeight: 500, letterSpacing: '-0.06em', lineHeight: 0.96, overflowWrap: 'anywhere' }}>{competition.title}</h2><p style={{ margin: 0, color: 'var(--muted)', lineHeight: 1.7, fontSize: '0.92rem', overflowWrap: 'anywhere' }}>{summary}</p></div>
        <div style={{ display: 'grid', alignContent: 'start', gap: '14px', padding: isMobile ? '14px' : '16px', borderRadius: '14px', background: 'rgba(0,0,0,0.14)', border: '1px solid rgba(255,255,255,0.08)', minWidth: 0 }}><div><span style={{ display: 'block', color: 'var(--muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Format</span><strong style={{ display: 'block', marginTop: '4px', lineHeight: 1.4, overflowWrap: 'anywhere' }}>One continuous Markdown record</strong></div><div><span style={{ display: 'block', color: 'var(--muted)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Photos</span><strong style={{ display: 'block', marginTop: '4px' }}>{gallery.length}</strong></div></div>
      </section>
